...
async def promote_user(bot: Client, msg: Message):
    ...
    await msg.reply_text(
        f"User {user_member.user.mention} has been promoted.",
        parse_mode="html",
    )
...
async def demote_user(bot: Client, msg: Message):
    ...
    await msg.reply_text(
        f"User {user_member.user.mention} has been demoted.", parse_mode="html"
    )
...
async def unpin_message(bot: Client, msg: Message):
    ...
    if target_msg is None:
        await msg.reply_text(
            "Please reply to a message to unpin it or use /unpinall."
        )
        return
    # unpin the message
...
async def get_invite_link(bot: Client, msg: Message):
    if msg.chat.type == "private":
        await msg.reply_text(
            "This command is meant to be used in a group chat."
        )
        return
    chat_id = msg.chat.id
    ...
    if not msg.chat.permissions.can_invite_users:
        await msg.reply_text(
            "I do not have permissions to generate invite links."
        )
        return
    await msg.reply_text(
        f"Group link: {await bot.export_chat_invite_link(chat_id)}"
    )
...
async def list_admins(bot: Client, msg: Message):
    ...
    if chat.id in admins:
        ...
        for admin in alist:
            reply = (
                reply
                + f" - {(await bot.get_chat_member(chat.id, admin)).user.mention}\n"
            )
        await msg.reply_text(reply)
    else:
        await msg.reply_text(
            "There are no admins in this chat,"
            + "this can't be right... Please do /reload!"
        )
