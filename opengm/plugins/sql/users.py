...
async def update_user(user_id, username, chat_id=None, chat_name=None):
    user = (await SESSION.execute(select(Users).where(Users.user_id == user_id))).scalars().first()
    if user is None:
    ...
    chat = (await SESSION.execute(select(Chats).where(Chats.chat_id == str(chat_id)))).scalars().first()
    if not chat:
    ...
    member = (await SESSION.execute(select(ChatMembers).where(ChatMembers.chat == chat.chat_id).where(ChatMembers.user == user.user_id))).scalars().first()
    if not member:
...
async def get_user_id_by_name(username):
    try:
        if username.startswith("@"):
            username = username[1:]
        id = (await SESSION.execute(select(Users).where(func.lower(Users.username) == username.lower()))).scalars().all() # .where(func.lower(Users.username) == username.lower())
        print("Here")
        return id
    finally:
        await SESSION.close()
